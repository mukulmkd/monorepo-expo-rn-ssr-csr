...
#import "EXJSIConversions.h"  // Include header first to get namespace definitions

// Include CallInvoker.h if available to get full definition (not just forward declaration)
#if __has_include(<ReactCommon/CallInvoker.h>)
#include <ReactCommon/CallInvoker.h>
#endif

// CallbackWrapper header path differs across RN distributions.
// VSCOReactNativeRuntime ships it under ReactCommon/...
...
{
#if __has_include(<ReactCommon/CallInvoker.h>)
  // Need to get a non-const reference to the function for move semantics
  jsi::Function func = value.getFunction(runtime);
  auto weakWrapper = react::CallbackWrapper::createWeak(std::move(func), runtime, jsInvoker);
  ...
  RCTResponseSenderBlock callback = ^(NSArray *responses) {
    ...
    // Use RN's non-virtual `invokeAsync(std::function<void()>)` to avoid direct virtual dispatch.
  ...
  return callback;
#else
  // CallInvoker.h not available - return a no-op callback
  // This should not happen in normal builds, but provides a fallback
  return ^(NSArray *responses) {
    // No-op: CallInvoker not available
  };
#endif
}
