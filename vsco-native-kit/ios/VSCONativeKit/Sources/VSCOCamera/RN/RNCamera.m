...
- (id)initWithBridge:(RCTBridge *)bridge
{
    if ((self = [super init])) {
        ...
        self.finishedReadingText = YES;
        self.finishedDetectingFace = YES;
...
-(void)handleDoubleTap:(UITapGestureRecognizer*)doubleTapRecognizer {
    [self handleTouch:doubleTapRecognizer isDoubleTap:YES];
}
-(void)handleTap:(UITapGestureRecognizer*)tapRecognizer {
    [self handleTouch:tapRecognizer isDoubleTap:NO];
}
    ...
    @try {
        [self.stillImageOutput captureStillImageAsynchronouslyFromConnection:connection completionHandler: ^(CMSampleBufferRef imageSampleBuffer, NSError *error) {
            if (imageSampleBuffer && !error) {
                ...
                // defaults to YES, must like Android
                bool writeExif = YES;
...
- (void)record:(NSDictionary *)options resolve:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject
{
    ...
    // note that mute will also be set to YES
    // if captureAudio is set to NO on the JS side.
...
- (void)initializeCaptureSessionInput
{
    ...
    dispatch_async(self.sessionQueue, ^{
        ...
        // recording a video
        // Only set it if captureAudio is YES so we don't prompt
