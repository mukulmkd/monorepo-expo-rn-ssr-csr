...
// Try to include JSI headers - if not available, we'll get a compile error
#if __has_include(<jsi/jsi.h>)
#include <jsi/jsi.h>
#else
// If <jsi/jsi.h> is not available, we can't use jsilib.h because it includes <jsi/jsi.h>
// We need to ensure JSI headers are available through the React product
// For now, this will cause a compile error that needs to be fixed
// The React product should expose JSI headers as <jsi/jsi.h>
#endif
