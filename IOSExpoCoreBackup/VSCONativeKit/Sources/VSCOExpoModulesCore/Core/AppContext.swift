  ...
  public weak var legacyModuleRegistry: EXModuleRegistry? {
    didSet {
      if let registry = legacyModuleRegistry,
        ...
        let fileSystemLegacyModule = legacyModule as? NSObject {
        // Call the method using performSelector to avoid compilation order issues with FileSystemLegacyUtilities
        if fileSystemLegacyModule.responds(to: Selector(("maybeInitAppGroupSharedDirectories:"))) {
          fileSystemLegacyModule.perform(Selector(("maybeInitAppGroupSharedDirectories:")), with: self.config.appGroupSharedDirectories)
        }
      }
    }
  }
