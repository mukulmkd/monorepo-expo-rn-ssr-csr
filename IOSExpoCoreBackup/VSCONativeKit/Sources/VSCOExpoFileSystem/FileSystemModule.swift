...
public final class FileSystemModule: Module {
  private lazy var sessionTaskDispatcher: EXSessionTaskDispatcher = {
    let handler = ExpoAppDelegateSubscriberRepository.getSubscriberOfType(FileSystemBackgroundSessionHandler.self)
    // Swift renames the protocol to EXSessionHandlerProtocol to avoid collision with the class
    // Cast to the protocol type
    return EXSessionTaskDispatcher(sessionHandler: handler as? EXSessionHandlerProtocol)
  }()
  ...
  public func definition() -> ModuleDefinition {
    ...
    AsyncFunction("getInfoAsync") { (url: URL, options: InfoOptions, promise: Promise) in
      ...
      guard !url.absoluteString.isEmpty else {
        promise.reject(Exception(name: "InvalidURL", description: "URL cannot be empty", code: "E_INVALID_URL"))
        return
      }
      
      // Capture resolver and rejecter to ensure they're available when the Objective-C handler calls them
      let resolver = promise.legacyResolver
      let rejecter = promise.legacyRejecter
      
      // Wrap the entire operation in a do-catch to ensure any exceptions are properly handled
      do {
        ...
        switch scheme {
        case "file":
          // Validate file URL has a path
          guard !url.path.isEmpty else {
            promise.reject(Exception(name: "InvalidFileURL", description: "File URL must have a valid path", code: "E_INVALID_FILE_URL"))
            return
          }
          
          // Ensure the URL is a valid file URL
          guard url.isFileURL else {
            promise.reject(Exception(name: "InvalidFileURL", description: "URL is not a valid file URL: \(url.absoluteString)", code: "E_INVALID_FILE_URL"))
            return
          }
          
          // Wrap the Objective-C handler call in a try-catch to catch any exceptions
          // The handler itself should call resolver/rejecter, but we catch any exceptions here
          do {
            EXFileSystemLocalFileHandler.getInfoForFile(url, withOptions: optionsDict, resolver: resolver, rejecter: rejecter)
          } catch {
            // If the handler call itself throws, reject the promise
            promise.reject(UnexpectedException(error))
          }
        case "assets-library", "ph":
          do {
            EXFileSystemAssetLibraryHandler.getInfoForFile(url, withOptions: optionsDict, resolver: resolver, rejecter: rejecter)
          } catch {
            promise.reject(UnexpectedException(error))
          }
        default:
          promise.reject(UnsupportedSchemeException(scheme.isEmpty ? "unknown" : scheme))
        }
    ...
    AsyncFunction("copyAsync") { (options: RelocatingOptions, promise: Promise) in
      ...
      if fromUrl.scheme == "file" {
        EXFileSystemLocalFileHandler.copy(from: fromUrl, to: toUrl, resolver: promise.legacyResolver, rejecter: promise.legacyRejecter)
      } else if ["ph", "assets-library"].contains(fromUrl.scheme) {
        EXFileSystemAssetLibraryHandler.copy(from: fromUrl, to: toUrl, resolver: promise.legacyResolver, rejecter: promise.legacyRejecter)
      } else {
        throw InvalidFileUrlException(fromUrl)
      }
    }
    ...
    AsyncFunction("downloadAsync") { (sourceUrl: URL, localUrl: URL, options: DownloadOptions, promise: Promise) in
      ...
      if sourceUrl.isFileURL {
        ...
        EXFileSystemLocalFileHandler.copy(from: sourceUrl, to: localUrl, resolver: promise.legacyResolver, rejecter: promise.legacyRejecter)
        return
      }
