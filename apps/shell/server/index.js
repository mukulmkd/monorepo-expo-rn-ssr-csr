...
const clientAliasPlugin = {
  ...
  setup(build) {
    ...
    // Ensure all React imports resolve to the same instance for client bundle
...
const serverAliasPlugin = {
  ...
  setup(build) {
    ...
    // Don't interfere with React - let it be external
...
// Static assets (client bundle placeholder)
app.use("/static", express.static(path.join(__dirname, "../dist/client")));

app.get("/health", (_req, res) => res.status(200).send("ok"));

// Proxy products API for web clients (avoid CORS)
...
app.use(async (req, res) => {
  try {
    // Import the built server renderer
...
app.listen(PORT, () => {
  console.log(`Shell SSR listening on http://localhost:${PORT}`);
});
