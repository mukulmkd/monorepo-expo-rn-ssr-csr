...
# Expo Modules Core (Android) source (vendored in-repo)
EXPO_CORE_SOURCE_DIR="${MONOREPO_ROOT}/expo-modules-core-source"
EXPO_CORE_ANDROID_DIR="${EXPO_CORE_SOURCE_DIR}/android"
EXPO_CORE_GRADLE_PLUGIN_DIR="${EXPO_CORE_SOURCE_DIR}/expo-module-gradle-plugin"

########################################
# Expo Modules Core (Android) helpers
########################################

get_expo_modules_core_version() {
    # Prefer the vendored source version, then fall back to root package.json.
    if [ -f "${EXPO_CORE_SOURCE_DIR}/package.json" ]; then
        node -e "const fs=require('fs'); const pkg=JSON.parse(fs.readFileSync('${EXPO_CORE_SOURCE_DIR}/package.json','utf8')); console.log(pkg.version);" 2>/dev/null \
          | tr -d '\r' \
          | sed 's/^[^0-9]*//; s/[^0-9.].*$//' \
          || true
        return 0
    ...
    node -e "const fs=require('fs'); const root=JSON.parse(fs.readFileSync('${ROOT_PACKAGE_JSON}','utf8')); const all={...(root.dependencies||{}),...(root.peerDependencies||{}),...(root.devDependencies||{})}; const v=(all['expo-modules-core']||'3.0.23'); console.log(String(v).replace(/[^0-9.]/g,''));" 2>/dev/null \
      | tr -d '\r' \
      || echo "3.0.23"
}

ensure_expo_module_gradle_plugin_published() {
    # Prepare expo-modules-core-source so expo-modules-core can be built from source without relying on
    # expo-module-gradle-plugin being published as a Maven artifact.
    #
    # We do this by:
    # 1) Ensuring expo-modules-core-source/node_modules/@react-native/gradle-plugin exists (symlink to monorepo root)
    # 2) Adding a composite build includeBuild("../expo-module-gradle-plugin") in expo-modules-core-source/android/settings.gradle
    #    so the buildscript classpath dependency `expo.modules:expo-module-gradle-plugin:<ver>` is substituted locally.

    if [ ! -d "$EXPO_CORE_GRADLE_PLUGIN_DIR" ]; then
        warn "expo-module-gradle-plugin source not found at: $EXPO_CORE_GRADLE_PLUGIN_DIR"
        return 1
    fi

    # Ensure the includeBuild path exists: expo-modules-core-source/node_modules/@react-native/gradle-plugin
    if [ ! -d "${EXPO_CORE_SOURCE_DIR}/node_modules/@react-native/gradle-plugin" ]; then
        if [ -d "${MONOREPO_ROOT}/node_modules/@react-native/gradle-plugin" ]; then
            mkdir -p "${EXPO_CORE_SOURCE_DIR}/node_modules"
            if [ ! -e "${EXPO_CORE_SOURCE_DIR}/node_modules/@react-native" ]; then
                ln -s "${MONOREPO_ROOT}/node_modules/@react-native" "${EXPO_CORE_SOURCE_DIR}/node_modules/@react-native" 2>/dev/null || true
            fi
        else
            warn "Missing ${MONOREPO_ROOT}/node_modules/@react-native/gradle-plugin - cannot publish expo-module-gradle-plugin reliably."
            warn "Run npm install in repo root first."
        fi
    fi

    # Ensure expo-modules-core android build uses the local expo-module-gradle-plugin via composite build.
    local settings_gradle="${EXPO_CORE_ANDROID_DIR}/settings.gradle"
    if [ -f "$settings_gradle" ]; then
        if ! grep -q 'includeBuild("../expo-module-gradle-plugin")' "$settings_gradle" 2>/dev/null; then
            log "  Wiring expo-modules-core Android build to use local expo-module-gradle-plugin (includeBuild)..."
            # Append at end (safe for Groovy settings.gradle).
            echo "" >> "$settings_gradle"
            echo "// Added by generate-native-kit-android.sh" >> "$settings_gradle"
            echo 'includeBuild("../expo-module-gradle-plugin")' >> "$settings_gradle"
        fi
    else
        warn "expo-modules-core settings.gradle not found at: $settings_gradle"
    fi

    return 0
}

build_expo_modules_core_aar() {
    if [ ! -d "$EXPO_CORE_ANDROID_DIR" ]; then
        err "expo-modules-core Android source not found at: $EXPO_CORE_ANDROID_DIR"
        return 1
    fi

    # Ensure required plugin is available.
    ensure_expo_module_gradle_plugin_published || true

    log "  Building expo-modules-core AAR from source..."
    # Keep output out of stdout-sensitive contexts (some callers use command substitution elsewhere).
    (cd "$EXPO_CORE_ANDROID_DIR" && ./gradlew :expo-modules-core:assembleRelease --no-daemon) 1>&2 || {
        err "Failed to build expo-modules-core AAR"
        return 1
    }

    local built_aar="${EXPO_CORE_ANDROID_DIR}/expo-modules-core/build/outputs/aar/expo-modules-core-release.aar"
    if [ ! -f "$built_aar" ]; then
        err "expo-modules-core AAR not found at expected path: $built_aar"
        return 1
    fi
    return 0
}

publish_expo_modules_core_to_maven_local() {
    local version="$1"
    local aar_path="$2"

    local m2_dir="${HOME}/.m2/repository/host/exp/exponent/expo-modules-core/${version}"
    local m2_aar="${m2_dir}/expo-modules-core-${version}.aar"
    local m2_pom="${m2_dir}/expo-modules-core-${version}.pom"

    mkdir -p "$m2_dir"
    cp "$aar_path" "$m2_aar"

    # Minimal POM. The consuming app already brings React Native via vsco-rn-host-sdk,
    # so we avoid trying to perfectly model all transitive dependencies here.
    cat > "$m2_pom" <<POM_EOF
...
<project xsi:schemaLocation="http://maven.apache.org/POM/4.0.0 http://maven.apache.org/xsd/maven-4.0.0.xsd"
  ...
  <artifactId>expo-modules-core</artifactId>
  <version>${version}</version>
  <packaging>aar</packaging>
</project>
POM_EOF

    log "  ✅ Published expo-modules-core:${version} to Maven Local (${m2_aar})"
}

ensure_expo_modules_core_dependency_ready() {
    # Deprecated: user requested NOT to publish expo-modules-core to mavenLocal.
    # Use ensure_expo_modules_core_aar_in_kit instead.
    return 0
}

ensure_expo_modules_core_aar_in_kit() {
    local version="$1"

    mkdir -p "${KIT_ANDROID_DIR}/libs"
    local kit_aar="${KIT_ANDROID_DIR}/libs/expo-modules-core.aar"
    local kit_aar_versioned="${KIT_ANDROID_DIR}/libs/expo-modules-core-${version}.aar"
    local built_aar="${EXPO_CORE_ANDROID_DIR}/expo-modules-core/build/outputs/aar/expo-modules-core-release.aar"

    # If already present, assume it's correct.
    if [ -f "$kit_aar" ]; then
        log "  ✅ expo-modules-core.aar already present in ${KIT_ANDROID_DIR}/libs"
        return 0
    fi

    build_expo_modules_core_aar || return 1
    if [ ! -f "$built_aar" ]; then
        err "expo-modules-core AAR not found at expected path: $built_aar"
        return 1
    fi

    cp "$built_aar" "$kit_aar"
    cp "$built_aar" "$kit_aar_versioned" 2>/dev/null || true

    # Also publish to Maven Local so consuming apps can resolve it via coordinates.
    publish_expo_modules_core_to_maven_local "$version" "$built_aar" || true

    log "  ✅ Copied expo-modules-core AAR into ${KIT_ANDROID_DIR}/libs (expo-modules-core.aar)"
    return 0
}
...
# If a previous run accidentally created AndroidManifest.xml as a directory, remove it.
if [ -d "$KIT_ANDROID_DIR/src/main/AndroidManifest.xml" ]; then
  rm -rf "$KIT_ANDROID_DIR/src/main/AndroidManifest.xml"
fi
...
# Determine whether any Expo modules are present (used for conditional generation steps).
...
# IMPORTANT: start from a clean generated sources state on each run.
# Otherwise stale files from previous runs can remain and cause Kotlin "Redeclaration" errors.
log "  Resetting generated Android sources (src/main/java, src/main/kotlin, src/main/res)..."
rm -rf "${KIT_ANDROID_DIR}/src/main/java" "${KIT_ANDROID_DIR}/src/main/kotlin" "${KIT_ANDROID_DIR}/src/main/res" 2>/dev/null || true
mkdir -p "${KIT_ANDROID_DIR}/src/main/java" "${KIT_ANDROID_DIR}/src/main/kotlin" "${KIT_ANDROID_DIR}/src/main/res"
...
UITHREADEXECUTOR_EOF
        ...
        # Fix 6: Ensure MetadataException is handled when reading EXIF via metadata-extractor (for MutableImage.java)
        # Directory.getInt(...) throws MetadataException; keep it in the catch union to compile.
        find "$android_java_dir" -type f -name "*.java" 2>/dev/null | while read -r file; do
            if grep -q "originalImageMetaData()" "$file" 2>/dev/null && grep -q "import com.drew.metadata.MetadataException" "$file" 2>/dev/null; then
                if ! grep -qE "catch\\s*\\([^)]*MetadataException" "$file" 2>/dev/null; then
                    perl -pi -e 's/catch\\s*\\(\\s*ImageProcessingException\\s*\\|\\s*IOException\\s+(\\w+)\\s*\\)/catch (ImageProcessingException | IOException | MetadataException $1)/g' "$file" 2>/dev/null || true
                fi
            fi
        done
        log "    ✅ Ensured MetadataException is handled in metadata-extractor call sites"
        ...
        #
        # NOTE: Disabled. This transform is brittle and has previously corrupted Java sources
        # (e.g. inserting try/catch blocks at invalid locations). Prefer adding proper Maven deps
        # (zxing/mlkit/metadata-extractor) instead of source rewriting.
        if false; then
        find "$android_java_dir" -type f -name "*.java" 2>/dev/null | while read -r file; do
...
detect_and_generate_stubs() {
    ...
    while IFS= read -r file; do
        ...
        grep -E "^import\s+[^;]+;" "$file" 2>/dev/null | \
            ...
            sed 's/^import\s*//;s/;\s*$//' | \
            # Skip system + known Maven-provided packages (we don't want to generate stubs for them)
            grep -vE "^(java|javax|android|androidx|com\.facebook\.react|com\.vsco\.nativekit|expo\.modules|com\.google\.zxing|com\.google\.mlkit|com\.drew)\." >> "$temp_imports" || true
    ...
    if [ -s "$temp_imports" ]; then
        sort -u "$temp_imports" | sed 's/^[[:space:]]*//;s/[[:space:]]*$//' | \
            grep -vE "^(java|javax|android|androidx|com\.facebook\.react|com\.vsco\.nativekit|expo\.modules|com\.google\.zxing|com\.google\.mlkit|com\.drew)\." | \
            ...
            while IFS= read -r import_line || [ -n "$import_line" ]; do
                ...
                if [ ! -f "$java_dir/$class_file" ] && [ ! -f "$kotlin_dir/$class_file_kt" ]; then
                    ...
                    if echo "$package_name" | grep -qE "^(java|javax|android|androidx|com\.facebook\.react|com\.vsco\.nativekit|expo\.modules|com\.google\.zxing|com\.google\.mlkit|com\.drew)"; then
                        continue
                    fi
            ...
            # Generate stub class
            {
                ...
                if [ "$is_interface" = true ]; then
                    ...
                    if echo "$class_name" | grep -qE "Exception$"; then
                        ...
                        # If this looks like a React Native module stub, implement NativeModule so it can be added to module lists.
                        if echo "$class_name" | grep -qE "Module$"; then
                            echo "public class ${class_name} implements NativeModule {"
                        else
                ...
                if echo "$class_name" | grep -qE "Module$" && [ "$is_interface" = false ]; then
                    # If we generated a Module stub, implement the NativeModule surface without @Override annotations
                    # to stay compatible across React Native versions.
...
# Step 4.1.5: Ensure react-native-camera optional async task classes exist
# Some builds of react-native-camera reference these tasks, but they may be missing depending on optional deps.
ensure_rn_camera_optional_task_stubs() {
    local tasks_dir="${KIT_ANDROID_DIR}/src/main/java/com/vsco/nativekit/camera/tasks"
    mkdir -p "$tasks_dir"

    # Minimal no-op AsyncTask implementations to satisfy compilation.
    # These intentionally do not perform detection; callers already gate usage via runtime flags.
    for task in FaceDetectorAsyncTask BarcodeDetectorAsyncTask TextRecognizerAsyncTask; do
        local task_file="${tasks_dir}/${task}.java"
        if [ ! -f "$task_file" ]; then
            cat > "$task_file" <<EOF
...
/**
 * Auto-generated stub task (optional dependency in some react-native-camera setups).
 * This is a no-op implementation used to satisfy compilation when optional ML/ZXing tasks are not present.
 */
...
# Only add these stubs if react-native-camera is bundled.
for dep in "${SHARED_NATIVE_DEPS[@]}"; do
    if [ "$dep" = "react-native-camera" ]; then
        ensure_rn_camera_optional_task_stubs
        break
    fi
done
...
# Step 4.9: Generate ExpoModulesPackageList (needed for Expo Kotlin modules at runtime)
########################################
if [ "$HAS_EXPO_MODULES" = true ]; then
    log "Step 4.9: Generating expo.modules.ExpoModulesPackageList..."
    expo_modules_dir="${KIT_ANDROID_DIR}/src/main/java/expo/modules"
    mkdir -p "$expo_modules_dir"

    # Build module + package lists based on detected expo-* deps.
    EXPO_MODULE_CLASSES=()
    EXPO_PACKAGE_INSTANCES=()

    # Expo core default modules (from expo-modules-core)
    EXPO_MODULE_CLASSES+=("expo.modules.kotlin.defaultmodules.CoreModule")
    EXPO_MODULE_CLASSES+=("expo.modules.kotlin.defaultmodules.JSLoggerModule")
    EXPO_MODULE_CLASSES+=("expo.modules.kotlin.defaultmodules.NativeModulesProxyModule")

    for dep in "${SHARED_NATIVE_DEPS[@]}"; do
        if [ "$dep" = "expo-file-system" ]; then
            EXPO_MODULE_CLASSES+=("com.vsco.nativekit.fileSystem.expo_modules_filesystem.FileSystemModule")
            EXPO_MODULE_CLASSES+=("com.vsco.nativekit.fileSystem.FileSystemNextModule")
            EXPO_PACKAGE_INSTANCES+=("com.vsco.nativekit.fileSystem.expo_modules_filesystem.FileSystemPackage()")
        fi
    done

    expo_pkg_list_file="${expo_modules_dir}/ExpoModulesPackageList.kt"
    cat > "$expo_pkg_list_file" <<'EOF'
...
EOF
    ...
    if [ ${#EXPO_MODULE_CLASSES[@]} -eq 0 ]; then
        echo "    listOf()" >> "$expo_pkg_list_file"
    else
        echo "    listOf(" >> "$expo_pkg_list_file"
        for cls in "${EXPO_MODULE_CLASSES[@]}"; do
            echo "      ${cls}::class.java," >> "$expo_pkg_list_file"
        done
        echo "    )" >> "$expo_pkg_list_file"
    fi
...
EOF
    ...
    if [ ${#EXPO_PACKAGE_INSTANCES[@]} -eq 0 ]; then
        echo "      listOf()" >> "$expo_pkg_list_file"
    else
        echo "      listOf(" >> "$expo_pkg_list_file"
        for pkg in "${EXPO_PACKAGE_INSTANCES[@]}"; do
            echo "        ${pkg}," >> "$expo_pkg_list_file"
        done
        echo "      )" >> "$expo_pkg_list_file"
    fi
...
EOF
    ...
    log "  ✅ Generated expo.modules.ExpoModulesPackageList"
fi
...
if [ "$HAS_EXPO_MODULES" = true ]; then
    # Prefer the vendored expo-modules-core-source version.
    EXPO_CORE_VERSION="$(get_expo_modules_core_version)"
    if [ -z "$EXPO_CORE_VERSION" ]; then
        EXPO_CORE_VERSION="3.0.23"
    fi

    # Build expo-modules-core AAR from source and place it into the kit's libs folder.
    # Then we'll reference it via `api(files("libs/expo-modules-core.aar"))`.
    ensure_expo_modules_core_aar_in_kit "$EXPO_CORE_VERSION" || {
        err "expo-modules-core AAR could not be built/copied into ${KIT_ANDROID_DIR}/libs."
        exit 1
    }

    log "  📦 Detected Expo modules - will use local libs/expo-modules-core.aar"
fi
...
dependencies {
    ...
    // expo-modules-core will be referenced as a local AAR in libs/ (compileOnly) when expo-* modules are detected
}
...
if [ "$HAS_EXPO_MODULES" = true ]; then
    # Prefer Maven Local coordinates for expo-modules-core so the consuming app gets a proper dependency graph.
    # (Local AAR file deps via api/implementation are forbidden when building an AAR; Maven deps are fine.)
    log "  Ensuring expo-modules-core Maven dependency is present in build.gradle..."

    # Remove any direct local AAR dependency line if it exists (legacy).
    perl -pi -e "s/^\\s*(api|implementation|compileOnly)\\s+files\\(\\s*['\\\"]libs\\/expo-modules-core\\.aar['\\\"]\\s*\\)\\s*\\n//g" "${KIT_ANDROID_DIR}/build.gradle" 2>/dev/null || true

    if ! grep -q "host\\.exp\\.exponent:expo-modules-core" "${KIT_ANDROID_DIR}/build.gradle" 2>/dev/null; then
        temp_gradle_expo=$(mktemp)
        in_dependencies=false
        brace_count=0
        inserted=false
            ...
            if [ "$in_dependencies" = true ]; then
                ...
                if [ "$brace_count" -eq 0 ] && [[ "$line" =~ }[[:space:]]*$ ]]; then
                    if [ "$inserted" = false ]; then
                        echo "    api 'host.exp.exponent:expo-modules-core:${EXPO_CORE_VERSION}'" >> "$temp_gradle_expo"
                        inserted=true
                    fi
                    in_dependencies=false
                fi
        ...
        if [ -f "$temp_gradle_expo" ] && [ -s "$temp_gradle_expo" ]; then
            mv "$temp_gradle_expo" "${KIT_ANDROID_DIR}/build.gradle"
            log "  ✅ Added api 'host.exp.exponent:expo-modules-core:${EXPO_CORE_VERSION}'"
        else
            warn "  ⚠️  Failed to inject expo-modules-core Maven dependency"
            rm -f "$temp_gradle_expo"
        fi
    else
        log "  ✅ expo-modules-core Maven dependency already present"
    fi
fi

# Ensure required runtime dependencies are exported transitively from the native-kit AAR.
# These are referenced directly by bundled sources (e.g. react-native-camera and expo-file-system).
# Using compileOnly causes runtime ClassNotFoundException crashes in consuming apps.
normalize_native_kit_runtime_deps() {
    local gradle_file="${KIT_ANDROID_DIR}/build.gradle"
    [ -f "$gradle_file" ] || return 0

    # Promote common runtime deps from compileOnly -> api (leave react-android/fresco as compileOnly).
    perl -pi -e "s/^\\s*compileOnly\\s+'commons-codec:commons-codec:/    api 'commons-codec:commons-codec:/g" "$gradle_file" 2>/dev/null || true
    perl -pi -e "s/^\\s*compileOnly\\s+'commons-io:commons-io:/    api 'commons-io:commons-io:/g" "$gradle_file" 2>/dev/null || true
    perl -pi -e "s/^\\s*compileOnly\\s+'androidx\\.documentfile:documentfile:/    api 'androidx.documentfile:documentfile:/g" "$gradle_file" 2>/dev/null || true
    perl -pi -e "s/^\\s*compileOnly\\s+'androidx\\.annotation:annotation:/    api 'androidx.annotation:annotation:/g" "$gradle_file" 2>/dev/null || true
    perl -pi -e "s/^\\s*compileOnly\\s+'androidx\\.exifinterface:exifinterface:/    api 'androidx.exifinterface:exifinterface:/g" "$gradle_file" 2>/dev/null || true
    perl -pi -e "s/^\\s*compileOnly\\s+'androidx\\.legacy:legacy-support-v4:/    api 'androidx.legacy:legacy-support-v4:/g" "$gradle_file" 2>/dev/null || true
    perl -pi -e "s/^\\s*compileOnly\\s+'com\\.drewnoakes:metadata-extractor:/    api 'com.drewnoakes:metadata-extractor:/g" "$gradle_file" 2>/dev/null || true
    perl -pi -e "s/^\\s*compileOnly\\s+'com\\.google\\.android\\.gms:play-services-mlkit-text-recognition:/    api 'com.google.android.gms:play-services-mlkit-text-recognition:/g" "$gradle_file" 2>/dev/null || true
    perl -pi -e "s/^\\s*compileOnly\\s+'com\\.google\\.zxing:core:/    api 'com.google.zxing:core:/g" "$gradle_file" 2>/dev/null || true

    # Expo Kotlin modules need kotlin-reflect at runtime for argument casting (Records) when codegen isn't available.
    # Only add this when Expo modules are actually present.
    if [ "${HAS_EXPO_MODULES:-false}" = true ] && ! grep -q "org\\.jetbrains\\.kotlin:kotlin-reflect" "$gradle_file" 2>/dev/null; then
        temp_gradle_reflect=$(mktemp)
        in_dependencies=false
        brace_count=0
        inserted=false
        ...
        while IFS= read -r line; do
            ...
            if [ "$in_dependencies" = true ]; then
                ...
                if [ "$brace_count" -eq 0 ] && [[ "$line" =~ }[[:space:]]*$ ]]; then
                    if [ "$inserted" = false ]; then
                        echo "    api 'org.jetbrains.kotlin:kotlin-reflect:2.1.0'" >> "$temp_gradle_reflect"
                        inserted=true
                    fi
                    in_dependencies=false
                fi
        ...
        if [ -f "$temp_gradle_reflect" ] && [ -s "$temp_gradle_reflect" ]; then
            mv "$temp_gradle_reflect" "$gradle_file"
        else
            rm -f "$temp_gradle_reflect"
        fi
    fi
}
...
# Create/update settings.gradle (not .kts - use Groovy for compatibility)
# Always (re)write so repo configuration stays in sync with detected deps.
cat > "${KIT_ANDROID_DIR}/settings.gradle" <<EOF
pluginManagement {
    repositories {
        mavenLocal()
        google()
